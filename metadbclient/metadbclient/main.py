...
def get_published_week_fields(published_week):
    '''
    Convert an ISO week of the format `YYYY-WW` into date filters.
    '''
    fields = {
        "published_date__gte" : [],
        "published_date__lte" : []
...
def get_published_week_range_fields(published_week_range):
    '''
    Convert an ISO week range of the format `[YYYY-WW, YYYY-WW]` into date filters.
    '''
    fields = {
        "published_date__gte" : [],
        "published_date__lte" : []
    ...
    * Writes user tokens to their tokens file, after running the provided method.
    ...
    def wrapped_method(obj, *args, **kwargs):
        if not obj.has_login_details:
            raise Exception("The client has no details to log in with. If you are using the client as a python import, please first log in with the 'get_login' command.")
        try:
...
class METADBClient:
    ...
    def __init__(self, config_dir_path=None, cli=False):
        # Locate the config
        config_dir_path, config_file_path = locate_config(config_dir_path=config_dir_path)
        ...
        self.endpoints = {
            ...
            "approve" : f"{self.url}/accounts/approve/", # TODO
            "institute-users" : f"{self.url}/accounts/institute-users/", # TODO
            "all-users" : f"{self.url}/accounts/all-users/", # TODO
    ...
    def get_login(self, username=None, use_password_env_var=False):
        ...
        if use_password_env_var:
            password_env_var = METADBClient.PASSWORD_ENV_VAR_PREFIX + self.username.upper() + METADBClient.PASSWORD_ENV_VAR_POSTFIX
        ...
        if response.status_code == 401:
            ...
            if not access_token_response.ok:
                ...
                if self.password is None:
                    print("Your refresh token has expired or is invalid. Please enter your password to request new tokens.")
    ...
    def create(self, pathogen_code, csv_path=None, fields=None, delimiter=None):
        ...
        if (csv_path is not None) and (fields is not None):
            raise Exception("Cannot provide both a csv file and fields dict at the same time")
...
def run():
    ...
    user_parser.add_argument("-u", "--username")
    ...
    # Get a list of the pathogen codes existing in the database
    ...
    if args.command == "make-config":
        ...
        if args.command == "register":
            ...
            client.get_login(username=args.username, use_password_env_var=args.use_password_env_var)
            ...
            if args.command == "create":
                if args.field is not None:
                    ...
                    if args.csv:
                        ...
                        client.create(args.pathogen_code, csv_path=args.tsv, delimiter="\t")
